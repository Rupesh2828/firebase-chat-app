...
import { formateDate, wrapEmojisInHtmlTag } from "@/utils/helpsers";
...
const Message = ({ message }) => {
  ...
  //by this self var. we r gonna identify which msg is send by logged in user
      ...
      >
        ...
        <div
          className={`group flex flex-col gap-4 p-4 rounded-3xl relative break-all ${
            self ? "rounded-br-md bg-c5" : "rounded-bl-mg bg-c1"
          }`}
        >
          {message.text && (
            <div
          ...
          {message.img && (
            <>
              ...
              {imageViewer && imageViewer.msdId === message.id && (
                <ImageViewer
                  src={[imageViewer?.url]}
                  currentIndex={0}
                  disableScroll={false}
